...
logging.basicConfig(filename='logs/v2_initial.log')
...
def main():
    ...
    ####
    e = eb.EarlyBird([x*0.05 for x in sov2.test_subnet_idx[1:]])
    ####
    if args.resume:
        ...
        try:
            for epoch in range(start_epoch, num_epochs):
                ####
                if not e.complete:
                    check_subnets(model, e, output_dir, args)
                ####
...
def check_subnets(model, e, saved_path, args):
    cfg_list = e.early_bird_emerge(model, _logger)
    sov2.subnet_chnls = [sov2.subnet_chnls[0]] + cfg_list
    sov2.cfgs = sov2.gen_subnet_cfgs()
    if args.local_rank==0:
        _logger.info('CFGs')
        _logger.info(str(sov2.subnet_chnls))
        with open(saved_path+"/cfgs.txt", "w") as output:
            output.write(str(sov2.subnet_chnls))
# additional subgradient descent on the sparsity-induced penalty term
def updateBN(model, epoch):
    # s = args.sparsity * ((math.cos(math.pi * (epoch/sov2.num_epochs)) + 1)/2)
    s = 0.0001
...
def train_one_epoch(
    ...
    model.train()
    resolutions = sov2.resolutions

    end = time.time()
    last_idx = len(loader) - 1
    num_updates = epoch * len(loader)
    num_subnets = len(sov2.subnet_chnls)
    _logger.info('number of subnetworks: {}'.format(num_subnets))
    for batch_idx, (input, target) in enumerate(loader):
        ####
        model.apply(lambda m: setattr(m, 'subnet_idx', 0))
        model.apply(lambda m: setattr(m, 'reso_idx', 0))
        ####
        ...
        if num_subnets > 1:
            sampled_width = random.sample(list(range(num_subnets)), 2)
        else:
            sampled_width = [0,0]
        subnet_list.extend(sampled_width)
        for subnet_idx in sorted(subnet_list):
            reso_idx = random.randint(0, len(sov2.resolutions)-1)  # sep-gate-pruning
            ...
            loss_scaler(loss, optimizer,clip_grad=args.clip_grad, clip_mode=args.clip_mode,
                ...
                create_graph=second_order, step=False)
            ################################################
        if num_subnets != len(sov2.test_subnet_idx):
        ...
        if loss_scaler is not None:
            loss_scaler(
                ...
                clip_grad=args.clip_grad, clip_mode=args.clip_mode,
                parameters=model_parameters(model, exclude_head='agc' in args.clip_mode),
                create_graph=second_order, backward=False)
        else:
...
def test(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='ALL', postloader=None, saved_path=''):
    ...
    with open(os.path.join(saved_path,'accuracy.csv'), 'w') as f:
        w = csv.writer(f)
        w.writerow(['Acc']+['{:.2f}'.format(1-0.05*subnet_idx) for subnet_idx in sorted(sov2.test_subnet_idx)])
