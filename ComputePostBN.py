...
def ComputeBN(net, postloader, resolution, num_batch=8, amp_autocast=None):
    net.train()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(postloader):
            ...
            with amp_autocast():
                _ = net(F.interpolate(inputs, (resolution, resolution), mode='bilinear', align_corners=True))
